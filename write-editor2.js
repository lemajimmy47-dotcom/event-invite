...
export default function CardEditorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  function drawCanvas() {
    ...
    if (card.card_host_text) {
      ...
      ctx.globalAlpha = 0.9
      ctx.fillText(card.card_host_text, W/2, y)
      y += 50
    }
    ...
    ctx.setLineDash([4, 4])
    ctx.beginPath()
    ctx.moveTo(W/2 - 120, y + 8)
    ctx.lineTo(W/2 + 120, y + 8)
    ctx.stroke()
    ctx.setLineDash([])
    ctx.globalAlpha = 1
    y += 35
    ...
    if (card.show_date && event?.date) {
      ...
      ctx.globalAlpha = 0.4
      ctx.beginPath()
      ctx.moveTo(W/2 - 100, y - 8)
      ctx.lineTo(W/2 - 30, y - 8)
      ctx.stroke()
      ctx.beginPath()
      ctx.moveTo(W/2 + 30, y - 8)
      ctx.lineTo(W/2 + 100, y - 8)
      ctx.stroke()
      ctx.globalAlpha = 1
      ctx.font = "bold 13px Arial"
      ctx.fillStyle = card.card_text_color
      ctx.fillText(days[d.getDay()].toUpperCase(), W/2 - 55, y)
      ctx.font = "bold 36px Georgia"
      ctx.fillStyle = card.name_color
      ctx.fillText(String(d.getDate()).padStart(2, "0"), W/2, y + 8)
      ctx.font = "bold 13px Arial"
      ctx.fillStyle = card.card_text_color
      ctx.fillText(String(d.getFullYear()), W/2 + 55, y)
      y += 20
    ...
    if (card.show_venue && event?.venue) {
      ...
      ctx.fillStyle = card.card_text_color
      ctx.globalAlpha = 1
      ctx.fillText(event.venue.toUpperCase(), W/2, y)
      y += 22
      if (event?.event_time) {
        ctx.font = "13px Arial"
        ctx.globalAlpha = 0.8
        ctx.fillText(event.event_time + " " + (event.time_period || ""), W/2, y)
        y += 30
      }
    }
    ...
    if (card.show_dress_code && event?.dress_code) {
      ...
      ctx.fillStyle = card.card_text_color
      ctx.globalAlpha = 0.7
      ctx.fillText("COLOR CODES", W/2, y)
      y += 18
      ctx.font = "bold 14px Georgia"
      ctx.fillStyle = card.name_color
      ctx.globalAlpha = 1
      ctx.fillText(event.dress_code, W/2, y)
      y += 20
      ...
      if (card.card_show_colors) {
        ...
        colors.forEach(color => {
          ...
          ctx.globalAlpha = 1
          ctx.beginPath()
          ctx.roundRect(bx, y, boxW, boxH, 4)
          ctx.fill()
          bx += boxW + 8
        })
        y += 35
    ...
    if (card.show_contacts && (event?.rsvp_contact1 || event?.rsvp_contact2 || event?.rsvp_contact3)) {
      ...
      ctx.fillStyle = card.card_text_color
      ctx.globalAlpha = 1
      ctx.fillText("Mawasiliano:", W/2, y)
      y += 20
      ctx.font = "13px Arial"
      ctx.globalAlpha = 0.85
      ...
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Designa kadi yako ya mwaliko</p>
          </div>
          <button onClick={saveSettings} disabled={saving} className="px-6 py-2.5 rounded-xl font-bold text-sm hover:opacity-90 disabled:opacity-50 transition shadow" style={{ background: "linear-gradient(135deg, #B8960C, #FFD700)", color: "#1a1200" }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
              <div className="p-3 border-b border-gray-100 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => setCard(s => ({ ...s, card_type: "SINGLE" }))} className={"px-3 py-1 rounded-lg text-xs font-bold transition " + (card.card_type === "SINGLE" ? "bg-yellow-400 text-yellow-900" : "bg-gray-100 text-gray-500")}>Single</button>
                  <button onClick={() => setCard(s => ({ ...s, card_type: "DOUBLE" }))} className={"px-3 py-1 rounded-lg text-xs font-bold transition " + (card.card_type === "DOUBLE" ? "bg-yellow-400 text-yellow-900" : "bg-gray-100 text-gray-500")}>Double</button>
                </div>
              </div>
              <div className="p-4">
                <canvas
                ...
                />
                <p className="text-xs text-gray-400 text-center mt-2">💡 Bonyeza na buruta QR code (fremu ya dhahabu) mahali unapotaka</p>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="flex gap-1 bg-gray-100 rounded-xl p-1">
              {[["content","📝 Maandishi"],["design","🎨 Design"],["position","📐 Nafasi"]].map(([id, label]) => (
            ...
            {activeTab === "content" && (
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 space-y-3">
                <h3 className="font-semibold text-gray-900 mb-2">📝 Taarifa za Kadi</h3>
                <div>
                  <label className={labelCls}>Maandishi ya Host</label>
                  <textarea rows={2} value={card.card_host_text} onChange={e => setCard(s => ({ ...s, card_host_text: e.target.value }))} className={inputCls} placeholder="Familia ya Christopher Mapunda wa Dar Es Salaam wanayofuraha kukualika" />
                </div>
                <div>
            ...
            {activeTab === "design" && (
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 space-y-4">
                <h3 className="font-semibold text-gray-900 mb-2">🎨 Design ya Kadi</h3>
            ...
            {activeTab === "position" && (
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 space-y-4">
                <h3 className="font-semibold text-gray-900 mb-2">📐 Nafasi ya QR Code</h3>
                <div>
                ...
                <div className="bg-blue-50 border border-blue-100 rounded-xl p-3">
                  <p className="text-xs text-blue-700">💡 Unaweza pia kuburuta QR code moja kwa moja kwenye picha ya kadi kushoto</p>
                </div>
              </div>
            )}
