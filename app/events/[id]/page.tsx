  ...
  return (
    <main className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto p-8">
        <Link href="/" className="text-blue-600 hover:underline text-sm">? Rudi nyumbani</Link>
                ...
                return (
                  <div key={guest.id} className="p-4 flex items-start justify-between gap-4">
                    <div className="flex-1">
                      ...
                      {guest.email && <p className="text-gray-400 text-sm">?? {guest.email}</p>}
                      ...
                      <div className="flex gap-2 mt-3 flex-wrap">
                        {guest.phone && <a href={waLink} target="_blank" rel="noopener noreferrer" className="text-xs bg-green-500 text-white px-3 py-1.5 rounded-lg hover:bg-green-600 transition">?? Tuma WhatsApp</a>}
                        <a href={qrLink} download={"QR-" + guest.name + ".png"} className="text-xs bg-blue-100 text-blue-700 px-3 py-1.5 rounded-lg hover:bg-blue-200 transition">? Download QR</a>
                      </div>
                    </div>
